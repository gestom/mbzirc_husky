...
typedef actionlib::SimpleActionServer<mbzirc_husky::brickRearrangeAction> Server;
Server *server;
...
{
	if(state == REARRANGING) return false;
    if(state == FINAL) return true;
	return true;
}

void actionServerCallback(const mbzirc_husky::brickRearrangeGoalConstPtr &goal, Server* as)
{
	mbzirc_husky::brickRearrangeResult result;
...
{
	ros::init(argc, argv, "brickRearrange");
	ros::NodeHandle n;
    pn = &n;
	...
	server = new Server(n, "brickRearrangeServer", boost::bind(&actionServerCallback, _1, server), false);
	server->start();
	while (ros::ok()){
