...
bool getPatternFront(mbzirc_husky_msgs::wall_pattern_close::Request &req, mbzirc_husky_msgs::wall_pattern_close::Response &res) {
  if (req.activate) {
...
bool getPatternAbove(mbzirc_husky_msgs::wall_pattern_close::Request &req, mbzirc_husky_msgs::wall_pattern_close::Response &res) {
  if (req.activate) {
    ...
    if (raise_arm.call(msg)) {
      ...
      if (got_img and got_height and got_params) {
        ...
        ostringstream s;
        ROS_INFO_STREAM("ERROR - unable to start subscribing" << endl
                                                              << "subs workin: "
                                                              << " image " << got_img << " height " << got_height << " cam params " << got_params);
        res.success = false;
        imageSub.shutdown();
        subHeight.shutdown();
        subInfo.shutdown();
...
void mainMouseCallback(int event, int x, int y, int flags, void *userdata) {
  ...
  if ((event == EVENT_LBUTTONUP || event == EVENT_LBUTTONUP)) {
    ...
    for (int i = 0; i < storedSamples.rows; i++)
      histogram.at<float>((int)storedSamples.at<float>(i, 0), (int)storedSamples.at<float>(i, 1))++;
